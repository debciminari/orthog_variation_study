#!/usr/bin/env python3
"""
Compute measures for intrinsic evaluation
"""
...
# ---- 2. cosine similarity-levenshtein distance ---------------------
...
def overall_pair_cosine(mats, tag, idx, pairs):
    """coseno medio complessivo, peso 1 per lemma."""
    by = defaultdict(list)
    for p in pairs:
        by[p["lemma"]].append(
...
# ---- zero-shot vs fine-tuning - cohesion ------------------------
...
def agg_seeds(per_seed_values):
    """lista di valori (uno per seed) -> (mean, std)."""
    a = np.array([v for v in per_seed_values if v == v])  # drop nan
    if len(a) == 0:
