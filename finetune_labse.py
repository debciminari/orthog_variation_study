...
def clsd_p_at_1(model, items):
    ...
    for it in items:
        ...
        sims = e_c @ e_src
        rank_true = int((sims > sims[0]).sum())   # 0 == top
        hit = int(rank_true == 0)
        correct += hit
        recs.append({
...
def run(args):
    ...
    train_ds = Dataset.from_dict({
        "rgn": [p[0] for p in pairs],
        "ita": [p[1] for p in pairs],
    })
    ...
    # curva di loss per il log verboso
...
def build_parser():
    ...
    ap.add_argument("--dev-cands", required=True,
                    help="CSV wide dev (rgn, ita, title, distractor_1..N)")
    ap.add_argument("--test-cands", required=True,
                    help="CSV wide test (rgn, ita, title, distractor_1..N)")
    ap.add_argument("--out-dir", default="out/runs")
    ap.add_argument("--src-col", default="rgn")
    ap.add_argument("--tgt-col", default="ita")
