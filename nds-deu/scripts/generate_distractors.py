...
SYSTEM_MSG = ("You are a native German speaker. "
              "You always reply with valid JSON and nothing else. "
              "All generated sentences MUST be written in Standard German.")

USER_TEMPLATE = """You are given a German sentence. Generate exactly {k} German sentences to be used as DISTRACTORS in a translation test.
...
- be SEMANTICALLY DIFFERENT: it must mean something DIFFERENT, so that it is NOT a
  ...
  quantity, or a named entity), while staying fluent and grammatical in German.

Do NOT paraphrase while keeping the meaning. Do NOT just swap synonyms.
IMPORTANT: every output sentence MUST be in Standard German, never in English.
...
# -----
ANTONYM_SWAPS = {
    r"\bimmer\b": "nie", r"\bnie\b": "immer",
    r"\bmehr\b": "weniger", r"\bweniger\b": "mehr",
    r"\balle\b": "keine", r"\bkeine\b": "alle",
    r"\bviel\b": "wenig", r"\bwenig\b": "viel",
    r"\bviele\b": "wenige", r"\bwenige\b": "viele",
    r"\bgroß\b": "klein", r"\bklein\b": "groß",
    r"\bgroße\b": "kleine", r"\bkleine\b": "große",
    r"\bvor\b": "nach", r"\bnach\b": "vor",
    r"\büber\b": "unter", r"\bunter\b": "über",
    r"\balt\b": "neu", r"\bneu\b": "alt",
    r"\balte\b": "neue", r"\bneue\b": "alte",
    r"\bgut\b": "schlecht", r"\bschlecht\b": "gut",
    r"\bhier\b": "dort", r"\bdort\b": "hier",
    r"\bimmer\b": "nie",
}

NEGATION_TRIGGERS = [
    (r"(?<!nicht )\b(ist)\b", r"\1 nicht"),
    (r"(?<!nicht )\b(sind)\b", r"\1 nicht"),
    (r"(?<!nicht )\b(war)\b", r"\1 nicht"),
    (r"(?<!nicht )\b(hat)\b", r"\1 nicht"),
    (r"(?<!nicht )\b(haben)\b", r"\1 nicht"),
    (r"(?<!nicht )\b(kann)\b", r"\1 nicht"),
    (r"(?<!nicht )\b(will)\b", r"\1 nicht"),
...
def perturb(sentence: str, max_out: int = 4):
    ...
    low = sentence.lower()
    if "nicht" not in low and "kein" not in low:
...
# --------------------------------------------------------------------------- #
def main():
    ap = argparse.ArgumentParser(description="Genera distrattori CLSD (nds,de).")
