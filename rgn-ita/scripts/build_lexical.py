#!/usr/bin/env python3
"""creation of five training sets with increasing variation"""
...
def variants_to_groups(variants_path):
    ...
    return {ita: {form: freq for form, freq in cells} for ita, cells in gl.items()}
...
def main():
    ...
    for lv in range(1,6):
        ...
        for i,row in enumerate(rows):
            ...
            out.append({"rgn":WORD_RE.sub(fn,row["rgn"]),"ita":row["ita"],"title":row["title"]})
        with open(out_dir/f"train_L{lv}.csv","w",encoding="utf-8",newline="") as f:
            w=csv.DictWriter(f,fieldnames=["rgn","ita","title"]); w.writeheader(); w.writerows(out)
        ch=sum(1 for a,b in zip(rows,out) if a["rgn"]!=b["rgn"])
