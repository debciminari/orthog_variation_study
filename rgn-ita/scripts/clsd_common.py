    ...
    try:
        ...
        if free_gb < 6.0:
            print(
                ...
                f"Qwen gira su Ollama). Se e' poco, un altro processo occupa memoria.",
                file=sys.stdout,
                flush=True,
            )
        return True
...
# ---------------------------------------------------------------------------
LLM_SYSTEM = (
    "You are an expert Italian linguist helping build an evaluation set of "
    "adversarial distractor sentences."
)
LLM_PROMPT_TMPL = (
    "Can you provide me with {n} tricky sentences (numbered) that look "
    "structurally and lexically similar but don't have the same meaning. The "
    "sentences should be within similar topics and share commonalities with the "
    "original. Answer in Italian!\n{sent}"
)


def _parse_json_list(txt: str) -> list[str]:
    """Parser tollerante: prova JSON puro, poi il primo blocco [...], poi righe."""
    ...
    if m:
        try:
            ...
            if isinstance(arr, list):
                return [norm(x) for x in arr if str(x).strip()]
...
class OllamaGenerator:
    ...
    def __init__(self, model_name: str, api_base: str = "http://localhost:11434",
        ...
        self.url = api_base.rstrip("/") + "/api/chat"
        self.max_tokens = max_tokens

    def __call__(self, system: str, user: str, temperature: float = 1.0) -> list[str]:
        ...
        try:
            ...
            if isinstance(obj, list):
                return [norm(x) for x in obj if str(x).strip()]
...
def make_generator(model_name: str, api_base: str = "http://localhost:11434"):
    return OllamaGenerator(model_name, api_base=api_base)


def llm_generate(sent: str, n: int, gen: Callable, temperature: float = 1.0) -> list[str]:
    return gen(LLM_SYSTEM, LLM_PROMPT_TMPL.format(n=n, sent=sent), temperature)
